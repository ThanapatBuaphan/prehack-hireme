...
export default function JobProfile() {
  ...
  const isOwner = id === "me" || String(myProfile?.userId) === id;
  ...
  const loadFull = async () => {
    setFullLoading(true);
    try {
      const url = isOwner ? "/api/user/profile" : `/api/user/profile/${id}`;
  ...
  const email = isOwner ? (myProfile?.email ?? "") : "";
    ...
    <div className="flex flex-col h-full min-h-screen bg-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              ...
              {isEditing && isOwner && <button onClick={() => setShowEduModal(true)} className="text-xs text-[#515DB6] hover:underline">+ Add</button>}
            </div>
            {educations.length === 0 && <p className="text-xs text-gray-400 italic">No education added</p>}
            {educations.map((edu) => (
              <CardSection key={edu.id} onDelete={isEditing && isOwner ? () => userService.deleteEducation(edu.id).then(loadFull) : undefined}>
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              ...
              {isEditing && isOwner && <button onClick={() => setShowWorkModal(true)} className="text-xs text-[#515DB6] hover:underline">+ Add</button>}
            </div>
            {workExps.length === 0 && <p className="text-xs text-gray-400 italic">No experience added</p>}
            {workExps.map((w) => (
              <CardSection key={w.id} onDelete={isEditing && isOwner ? () => userService.deleteWorkExperience(w.id).then(loadFull) : undefined}>
            ...
            <div className="flex items-center gap-3 border border-gray-200 rounded-xl p-3">
              ...
              <div className="flex-1 text-sm min-w-0">
                {resumeFile
                  ...
                  : data.resume
                  ? <a href={data.resume} target="_blank" rel="noopener noreferrer" className="text-[#515DB6] underline">Resume.pdf</a>
      ...
      {showEduModal && (
        <AddEducationModal onClose={() => setShowEduModal(false)}
          onSave={async (d) => { await userService.addEducation(d); await loadFull(); }} />
      )}
      {showWorkModal && (
        <AddWorkModal onClose={() => setShowWorkModal(false)}
          onSave={async (d) => { await userService.addWorkExperience(d); await loadFull(); }} />
      )}
    </div>
  );
}