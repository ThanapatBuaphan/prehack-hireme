...
export async function userRegister(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
        ...
        res.status(201).json({ token, role: "user", profileId: result.user.id, email });
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const company = await tx.company.create({
                data: { email, companyName, createdAt: new Date(), accountId: account.id },
            });
            return { account, company };
        });

        const token = generateToken(result.account, result.company.id);
        res.status(201).json({ token, role: "company", profileId: result.company.id, email });
...
export async function login(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
        ...
        const profileId = account.role === "user" ? account.user?.id : account.company?.id;
        ...
        res.json({ token, role: account.role, profileId, email });
